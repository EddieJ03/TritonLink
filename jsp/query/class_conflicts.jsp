                    ...
                    <%
                        ...
                        try {
                            ...
                            if (action != null && action.equals("find")) {
                                ...
                                statement = conn.prepareStatement(("WITH EnrolledMeetings AS (     SELECT C.SECTION_ID as section_id,          C.COURSE_NUMBER as course_number,          day_of_week, start_time, end_time,          C.title as title      FROM enrolled E          JOIN student s ON s.PID = e.PID          JOIN Classes C ON E.SECTION_ID = C.SECTION_ID AND E.COURSE_NUMBER = C.COURSE_NUMBER          JOIN weekly_meeting wm ON c.SECTION_ID = wm.SECTION_ID AND c.COURSE_NUMBER = wm.COURSE_NUMBER      WHERE S.ssn = '987-65-4321' AND C.year = 2018 AND C.quarter = 'Spring'), NonenrolledMeetings AS (     SELECT C.section_id as section_id, C.title as title,      wm.course_number as course_number, day_of_week, start_time, end_time      FROM weekly_meeting wm JOIN Classes C ON wm.SECTION_ID = C.SECTION_ID AND wm.COURSE_NUMBER = C.COURSE_NUMBER      WHERE (C.SECTION_ID, C.COURSE_NUMBER) NOT IN (          SELECT section_id, course_number FROM EnrolledMeetings) AND C.year = 2018 AND C.quarter = 'Spring')  SELECT DISTINCT em.title as enrolled_title,      em.course_number as enrolled_course,      nem.title as not_enrolled_title,      nem.course_number as not_enrolled_course  FROM (EnrolledMeetings em JOIN classes cl ON em.section_id = cl.section_id AND em.course_number = cl.course_number) CROSS JOIN (NonenrolledMeetings nem JOIN classes cla ON nem.section_id = cla.section_id AND nem.course_number = cla.course_number)     WHERE em.day_of_week = nem.day_of_week AND cla.quarter = cl.quarter AND cl.year = cla.year         AND ((em.section_id <> nem.section_id) OR (em.course_number <> nem.course_number))         AND (em.start_time, em.end_time) OVERLAPS (nem.start_time, nem.end_time);"));
                            ...
                            ResultSet pidRS = pidStatement.executeQuery("SELECT DISTINCT ssn, first_name, middle_name, last_name FROM student JOIN enrolled ON student.PID = enrolled.PID JOIN classes on classes.section_id = enrolled.section_id AND classes.course_number = enrolled.course_number WHERE classes.year = 2018 AND classes.quarter = 'Spring'");
                            //
                    %>
                    <table>
                        <tr>
