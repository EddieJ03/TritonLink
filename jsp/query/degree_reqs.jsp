                    ...
                    <%
                        ...
                        try {
                            ...
                            if (action != null && action.equals("find")) {
                                ...
                                statementDegree = conn.prepareStatement(("SELECT d.degree_id, d.degree_type, GREATEST(d.total_units - COALESCE(SUM(e.num_units), 0), 0) AS remaining_units, d.total_units FROM degree d JOIN category c ON d.degree_id = c.degree_id JOIN category_course cc ON c.category_id = cc.category_id JOIN course co ON cc.course_number = co.course_number LEFT JOIN enrolled e ON co.course_number = e.course_number AND e.PID = ? AND e.grade NOT IN ('F', 'U', 'Incomplete') WHERE c.degree_id = ? AND c.is_concentration = FALSE GROUP BY d.degree_id;"));
                                statementDegree.setString(1, request.getParameter("PID"));
                                statementDegree.setString(2, request.getParameter("degree_id"));

                                statementCategoryRemaining = conn.prepareStatement(("SELECT c.degree_id, c.category_name, GREATEST(c.required_units - COALESCE(SUM(e.num_units), 0), 0) AS remaining_units, c.required_units FROM category c JOIN category_course cc ON c.category_id = cc.category_id JOIN course co ON cc.course_number = co.course_number LEFT JOIN enrolled e ON co.course_number = e.course_number AND e.PID = ? AND e.grade NOT IN ('F', 'U', 'Incomplete') WHERE c.degree_id = ? AND c.is_concentration = FALSE GROUP BY c.degree_id, c.category_id;"));
